...
class LlmLib(ABC):
    ...
    def conversation_to_metadata(self, conversation: Conversation, generateEmbeddings=False) -> RawMetadata|None:
        ...
        # Cap before the validator's random cull: at 20+ cleaned tags
        # validate_tag_set keeps a random 20 and discards the rest
        # (LlmLib.validate_tag_set:198). This is the stock/fallback path; the
        # sn33 layer ranks and selects properly before it ever gets here.
        #
        # An earlier version of this hook ranked tags by cosine to the
        # conversation, but did it with one synchronous embeddings request PER
        # TAG - 20-30 sequential round trips inside a 12s synapse budget - and
        # benchmarked no better than the plain cap (0.5625 vs 0.5661). See
        # sn33/tags.py for the batched version that replaced it.
        tags = tags[:19]
